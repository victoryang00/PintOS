...
.*/
static bool parse_file_path(const char *name, struct dir **dirp, char base_name[NAME_MAX + 1]) {
    struct dir *dir = NULL;
    struct inode *inode;
    const char *cp;
...
filesys_create (const char *name, off_t initial_size) 
{
    ...
    bool success = (parse_file_path(name, &dir, base_name) && free_map_allocate(1, &inode_sector));
    if (success) {
        struct inode *inode;
...
{
  int file_name_state = name[0] == '/'*2 + name[strspn (name, "/")] == '\0';
  switch(file_name_state){
    case 3:
...
move:
   ...
   char base_name[NAME_MAX + 1];
   success = parse_file_path(name, &dir, base_name);
   if (success) {
...
do_format (void)
{
  ...
  struct inode *temp = dir_create(ROOT_DIR_SECTOR, ROOT_DIR_SECTOR);
  if (temp == NULL)
