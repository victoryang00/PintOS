...
process_execute (const char *file_name) 
{
   struct get_ready 
  {
    ...
    struct wsem *wait_status;    /* Child process. */
    bool success;                       /* Program successfully loaded? */
  };
  struct get_ready exec;
  char thread_name[16];
  char *save_ptr;

  tid_t tid;

  /* Initialize get_ready. */
  exec.file_name = file_name;
  sema_init(&exec.load_done, 0);

  /* Create a new thread to execute FILE_NAME. */
  strlcat(thread_name, file_name, sizeof thread_name);
  strtok_r(thread_name, " ", &save_ptr);
  tid = thread_create(thread_name, PRI_DEFAULT, start_process, &exec);
  if (tid != TID_ERROR) {
      sema_down(&exec.load_done);
      if (exec.success)
        list_push_back(&thread_current ()->child_list, &exec.wait_status->elem);
      else 
        tid = TID_ERROR;
  }
  return tid;
...
start_process (void *file_name_)
{
  struct get_ready 
  {
    ...
    struct get_ready *exec = file_name_;
  ...
  while (success)
    {
      exec->wait_status = thread_current ()->wsem
...
process_exit (void)
{
         ...
         e = list_next (e))
    {
        ...
        /* Tell all of its child process that they are free to exit because their parent process is dead. There is no
         parent process need to wait for their exit status.. */
        sema_up (&child->tsem);
    }

    /* try to exit completely . */
    sema_down(&cur->tsem);
...
load (const char *cmd_line, void (**eip) (void), void **esp) 
{
  ...
  while (*cmd_line == ' ')
    cmd_line++;
    strlcat(file_name, cmd_line, sizeof file_name);
    cp = strchr(file_name, ' ');
  if (cp != NULL)
    *cp = '\0';
